...
const active = ['&:active', '&[data-active=true]'].join(',')
const checked = ['&[aria-checked=true]', '&:checked'].join(',')
const selected = [
	'&[aria-selected=true]',
	'&[data-selected=true]',
...
const empty = '&:empty'
const enabled = ['&:enabled', '&:enabled:focus', '&:enabled:hover'].join(',')
...
const valid = ['&[aria-invalid=false]', '&:valid'].join(',')
const invalid = ['&[aria-invalid=true]', '&:invalid'].join(',')
const lastChild = '&:last-child'
const lastOfType = '&:last-of-type'
const nthOfTypeFn = createPseudoClassTransformFunction('&:nth-of-type')
const readOnly = ['&[aria-readonly=true]', '&:read-only'].join(',')
